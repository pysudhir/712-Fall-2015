...
Status Truncate(const std::string& filename, uint64_t length) {
  ...
  Status s = env->NewSequentialFile(filename, &orig_file, false, "");
  if (!s.ok())
    return s;
  ...
  if (s.ok()) {
    ...
    s = env->NewWritableFile(tmp_name, &tmp_file, false, "");
    if (s.ok()) {
...
Status FaultInjectionTestEnv::NewWritableFile(const std::string& fname,
  ...
  Status s = target()->NewWritableFile(fname, &actual_writable_file, false, "");
  if (s.ok()) {
